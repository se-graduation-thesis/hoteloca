...
const participantRef = firebase.database().ref().child("participants");
...
export const initializeListensers = async (userId) => {
  ...
  currentUserRef.child("offers").on("child_added", async (snapshot) => {
    ...
    if (data?.offer) {
      console.log("aaa", store.getState().meet)
      const pc = store.getState().meet.participants[data.offer.userId].peerConnection;
  ...
  currentUserRef.child("offerCandidates").on("child_added", (snapshot) => {
    const data = snapshot.val();
    if (data.userId) {
      const pc = store.getState().meet.participants[data.userId].peerConnection;
  ...
  currentUserRef.child("answers").on("child_added", (snapshot) => {
    ...
    if (data?.answer) {
      const pc =
        store.getState().meet.participants[data.answer.userId].peerConnection;
...
const createAnswer = async (otherUserId, userId) => {
  const pc = store.getState().meet.participants[otherUserId].peerConnection;
